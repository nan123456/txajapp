...
//				alert(data);
				if (data.result=='success') {					
					...
					if(data.myflag == "error"){
						return callback('该账号对应的手机号已被冻结,将不能登录!');
					} else {
	...
	owner.reg = function(regInfo, callback) {
		...
		if (regInfo.fgsxz=="") {
			return callback('请填写公司');
		}
		if (regInfo.nameX=="") {
			return callback('请填写姓名');
		}
//		if (regInfo.bmxz=="") {
//			return callback('请填写部门');
//		}
			...
			mui.ajax(url+'findEmail.php',{
				...
				success:function(data){
					...
					for (var i=0;i<length-1;i++) {
						var passwordFind=data[i].密码;
						findPassword.setAttribute("class","mui-input-row");
						passwordfind.value=passwordFind;
					}
